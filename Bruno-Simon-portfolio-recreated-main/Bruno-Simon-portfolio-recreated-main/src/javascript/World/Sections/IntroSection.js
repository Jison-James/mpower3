...
export default class IntroSection {
    ...
    setGroup5Sign() {
        ...
        context.fillText('group5', 512, 256)
        ...
        // Plane Mesh
        const boardPlaneResource = this.resources.items.projectsBoardPlane.scene.children[0]
        const mesh = boardPlaneResource.clone()
        mesh.position.set(x, y, 0)
        mesh.matrixAutoUpdate = false
        mesh.updateMatrix()

        // Material
        mesh.material = new ProjectBoardMaterial()
        mesh.material.uniforms.uColor.value = new THREE.Color('#8e7161')
        mesh.material.uniforms.uTexture.value = texture
        mesh.material.uniforms.uTextureAlpha.value = 1
    ...
    setStatic() {
        ...
        // Remove obstructing tree/bush
        const boardPosition = new THREE.Vector3(-6, -3.5, 0)
        this.resources.items.introStaticBase.scene.traverse((_child) => {
            if (_child instanceof THREE.Mesh) {
                const distance = _child.position.distanceTo(boardPosition)
                if (distance < 8 && (_child.name.toLowerCase().includes('sapin') || _child.name.toLowerCase().includes('tree') || _child.name.toLowerCase().includes('vegetation') || _child.name.toLowerCase().includes('plant') || _child.name.toLowerCase().includes('buisson') || _child.name.toLowerCase().includes('bush'))) {
                    _child.visible = false
                    _child.scale.set(0, 0, 0)
                    _child.position.set(0, 0, -100)
                    _child.updateMatrix()
                }
            }
        })
    ...
    setGroupBranding() {
        ...
        // Image/Texture
        const texture = this.resources.items.groupLogoTexture
